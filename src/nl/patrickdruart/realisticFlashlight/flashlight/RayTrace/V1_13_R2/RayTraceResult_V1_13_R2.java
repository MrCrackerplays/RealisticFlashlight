package nl.patrickdruart.realisticFlashlight.flashlight.RayTrace.V1_13_R2;
...
import org.bukkit.craftbukkit.v1_13_R2.CraftFluidCollisionMode;
 ...
 */
public class RayTraceResult_V1_13_R2 implements IRayTrace {
	 ...
	 * @param result
	 */
	public RayTraceResult_V1_13_R2(org.bukkit.util.RayTraceResult result) {
		this.result = result;
	}
	 ...
	 */
	public RayTraceResult_V1_13_R2(Vector hitPosition) {
	 ...
	 */
	public RayTraceResult_V1_13_R2(Vector hitPosition, BlockFace hitBlockFace) {
	 ...
	 * @param hitBlockFace the hit block face
	 */
	public RayTraceResult_V1_13_R2(Vector hitPosition, Block hitBlock, BlockFace hitBlockFace) {
	 ...
	 */
	public RayTraceResult_V1_13_R2(Vector hitPosition, Entity hitEntity) {
	 ...
	 * @param hitBlockFace the hit block face
	 */
	public RayTraceResult_V1_13_R2(Vector hitPosition, Entity hitEntity, BlockFace hitBlockFace) {
		result = new org.bukkit.util.RayTraceResult(hitPosition, hitEntity, hitBlockFace);
	}

	public static RayTraceResult_V1_13_R2 rayTraceEntities(Location start, Vector direction, double maxDistance) {
	...
	public static RayTraceResult_V1_13_R2 rayTraceEntities(Location start, Vector direction, double maxDistance,
		...
		return (nearestHitEntity == null) ? null
				: new RayTraceResult_V1_13_R2(nearestHitResult.getHitPosition(), nearestHitEntity,
						nearestHitResult.getHitBlockFace());
	}

	public static RayTraceResult_V1_13_R2 rayTraceBlocks(Location start, Vector direction, double maxDistance) {
	...
	public static RayTraceResult_V1_13_R2 rayTraceBlocks(Location start, Vector direction, double maxDistance,
		...
		MovingObjectPosition nmsHitResult = world.getHandle().rayTrace(startPos, endPos,
				CraftFluidCollisionMode.toNMS(mode), ignorePassableBlocks, false);
		...
		if (FlashlightPlugin.getConfigurationManager().getConfiguration("config")
			...
			while (result != null && result.getHitBlock() != null && result.getHitPosition() != null
				...
				nmsHitResult = world.getHandle().rayTrace(startPos, endPos, CraftFluidCollisionMode.toNMS(mode),
		...
		return (result == null) ? null : new RayTraceResult_V1_13_R2(result);
	}

	public static RayTraceResult_V1_13_R2 iteratorBlocks(Location start, Vector direction, double maxDistance,
			FluidCollisionModeFlash fluidCollisionMode, boolean ignorePassableBlocks) {
		BlockIterator blIt = new BlockIterator(start.getWorld(), start.toVector(), direction, 0d, (int) maxDistance);
		RayTraceResult_V1_13_R2 result = null;
		while (blIt.hasNext()) {
			...
			if ((block.isLiquid() && fluidCollisionMode != FluidCollisionModeFlash.NEVER)
				...
				result = new RayTraceResult_V1_13_R2(block.getLocation().toVector());
				break;
			}
		}
		return result;
