...
// taken from spigot 1.13+ source due to older versions not containing it
/**
...
public class BoundingBox implements Cloneable, ConfigurationSerializable {
	...
	/**
	 * Creates a new bounding box using the coordinates of the given locations as
	 * corners.
	 *
	 * @param corner1 the first corner
	 * @param corner2 the second corner
	...
	public static BoundingBox of(Block block) {
		...
		return new BoundingBox(block.getX(), block.getY(), block.getZ(), block.getX() + 1, block.getY() + 1,
				block.getZ() + 1);
	}
	...
	public static BoundingBox of(Vector center, double x, double y, double z) {
		...
		return new BoundingBox(center.getX() - x, center.getY() - y, center.getZ() - z, center.getX() + x,
				center.getY() + y, center.getZ() + z);
	}
	...
	public BoundingBox copy(BoundingBox other) {
		...
		return this.resize(other.getMinX(), other.getMinY(), other.getMinZ(), other.getMaxX(), other.getMaxY(),
				other.getMaxZ());
	}
	 ...
	 * directions.
	 * <p>
	 * Negative values will shrink the bounding box in the corresponding direction.
	 * Shrinking will be limited to the point where the affected opposite faces
	 * would meet if the they shrank at uniform speeds.
	 *
	 ...
	 */
	public BoundingBox expand(double negativeX, double negativeY, double negativeZ, double positiveX, double positiveY,
			double positiveZ) {
	...
	/**
	 * Expands this bounding box uniformly by the given values in both positive and
	 * negative directions.
	 * <p>
	 * Negative values will shrink the bounding box. Shrinking will be limited to
	 * the bounding box's current size.
	 *
	 * @param x the amount of expansion in both positive and negative x direction
	 * @param y the amount of expansion in both positive and negative y direction
	 * @param z the amount of expansion in both positive and negative z direction
	 * @return this bounding box (now expanded)
	 */
	 ...
	 * <p>
	 * A negative value will shrink the bounding box. Shrinking will be limited to
	 * the bounding box's current size.
	 *
	 ...
	 * <p>
	 * The magnitude of the direction will scale the expansion. A negative expansion
	 * value will shrink the bounding box in this direction. Shrinking will be
	 * limited to the bounding box's current size.
	 *
	 ...
	 * <p>
	 * A negative expansion value will shrink the bounding box in this direction.
	...
	public BoundingBox expand(BlockFace blockFace, double expansion) {
		...
		Vector vec = new Vector(blockFace.getModX(), blockFace.getModY(), blockFace.getModZ());
		if (blockFace.getModX() != 0 || blockFace.getModY() != 0 || blockFace.getModZ() != 0)
			vec.normalize();
		return this.expand(vec, expansion);
	}
	 ...
	 * positive values will expand it in the positive direction. The magnitudes of
	 * the direction components determine the corresponding amounts of expansion.
	 *
	 * @param dirX the x direction component
	 * @param dirY the y direction component
	 * @param dirZ the z direction component
	 ...
	 * <p>
	 * Negative values will expand the bounding box in the negative direction,
	 * positive values will expand it in the positive direction. The magnitude of
	 * the direction vector determines the amount of expansion.
	 *
	...
	/**
	 * Resizes this bounding box to represent the intersection of this and the given
	 * bounding box.
	 *
	 * @param other the other bounding box
	 ...
	 * <p>
	 * Bounding boxes that are only intersecting at the borders are not considered
	 * overlapping.
	 *
	 * @param other the other bounding box
	...
	/**
	 * Checks if this bounding box overlaps with the bounding box that is defined by
	 * the given corners.
	 * <p>
	 * Bounding boxes that are only intersecting at the borders are not considered
	 * overlapping.
	 ...
	 * <p>
	 * Positions exactly on the minimum borders of the bounding box are considered
	 * to be inside the bounding box, while positions exactly on the maximum borders
	 * are considered to be outside. This allows bounding boxes to reside directly
	 * next to each other with positions always only residing in exactly one of
	 * them.
	 *
	 ...
	 *
	 * @param other the other bounding box
	 * @return <code>true</code> if the bounding box contains the given bounding box
	 */
	...
	/**
	 * Checks if this bounding box fully contains the bounding box that is defined
	 * by the given corners.
	 *
	 * @param min the first corner
	 * @param max the second corner
	 * @return <code>true</code> if the bounding box contains the specified bounding
	 *         box
	 */
	 ...
	 * segment.
	 * <p>
	 * Intersections at edges and corners yield one of the affected block faces as
	 * hit result, but it is not defined which of them.
	 *
	 ...
	 */
	public IRayTrace rayTrace(Vector start, Vector direction, double maxDistance) {
		...
		return IRayTrace.getRayTraceResult(hitPosition, hitBlockFace);
	}
